...
/**
 * Utility class for adding or removing annotation to/from class at runtime.
 *
...
public final class RuntimeAnnotations {
    ...
    public static <T extends Annotation> void addAnnotation(Class<?> c, Class<T> annotationClass,
            Map<String, Object> valuesMap) {
        addAnnotation(c, annotationClass, annotationForMap(annotationClass, valuesMap));
    }

    public static <T extends Annotation> void addAnnotation(Class<?> c, Class<T> annotationClass, T annotation) {
        try {
            while (true) { // retry loop
                ...
                Object newAnnotationData = addToAnnotationData(annotationData, annotationClass, annotation,
                                                                classRedefinedCount);
                // try to install it
    ...
    public static <T extends Annotation> void removeAnnotation(Class<?> c, Class<T> annotationClass) {
        try {
            while (true) { // retry loop
                int classRedefinedCount = Class_classRedefinedCount.getInt(c);
                Object /*AnnotationData*/ annotationData = Class_annotationData.invoke(c);
                // null or stale annotationData -> optimistically create new instance
                Object newAnnotationData = removeFromAnnotationData(annotationData, annotationClass, classRedefinedCount);
                // try to install it
                if ((boolean) Atomic_casAnnotationData.invoke(ATOMIC_CLASS, c, annotationData, newAnnotationData)) {
                    // successfully installed new AnnotationData
                    break;
    ...
    private static <T extends Annotation> Object /*AnnotationData*/ addToAnnotationData(
    ...
    @SuppressWarnings("unchecked")
    private static <T extends Annotation> Object /*AnnotationData*/ removeFromAnnotationData(
                    Object /*AnnotationData*/ annotationData, Class<T> annotationClass,
                    int classRedefinedCount) throws InstantiationException, IllegalAccessException,
                                                    IllegalArgumentException, InvocationTargetException {
        Map<Class<? extends Annotation>, Annotation> annotations = (Map<Class<? extends Annotation>, Annotation>) AnnotationData_annotations
                .get(annotationData);
        ...
        Map<Class<? extends Annotation>, Annotation> newDeclaredAnnotations = new LinkedHashMap<>(annotations);
        if (newDeclaredAnnotations.containsKey(annotationClass)) {
            newDeclaredAnnotations.remove(annotationClass);
        }
        Map<Class<? extends Annotation>, Annotation> newAnnotations;
        if (declaredAnnotations == annotations) {
            newAnnotations = newDeclaredAnnotations;
